...
impl<W: LayoutElement> ScrollingSpace<W> {
    ...
    fn compute_new_view_offset_fit(
        ...
        let target_x = target_x.unwrap_or_else(|| self.target_view_main_pos());
    ...
    fn compute_new_view_offset_for_column(
        ...
        match self.options.layout.center_focused_column {
            ...
            CenterFocusedColumn::OnOverflow => {
                ...
                let source_col_x = self.column_main_pos(source_idx);
                let source_col_width = self.columns[source_idx].width();

                let target_col_x = self.column_main_pos(idx);
    ...
    fn animate_view_offset_with_config(
        ...
        let new_col_x = self.column_main_pos(idx);
        let old_col_x = self.column_main_pos(self.active_column_idx);
    ...
    pub(super) fn insert_position(&self, pos: Point<f64, Logical>) -> InsertPosition {
        ...
        let (closest_tile_idx, closest_tile_cross) = if col.display_mode == ColumnDisplay::Tabbed {
    ...
    fn move_column_to(&mut self, new_idx: usize) {
        ...
        let current_col_x = self.column_main_pos(self.active_column_idx);
        let next_col_x = self.column_main_pos(self.active_column_idx + 1);
        ...
        let view_offset_delta = -self.column_main_pos(self.active_column_idx) + current_col_x;
        ...
        let new_col_x = self.column_main_pos(new_idx);
    ...
    pub fn consume_into_column(&mut self) {
        ...
        offset.x += self.column_main_pos(source_column_idx);
        offset.x -= self.column_main_pos(target_column_idx);
    ...
    pub fn center_visible_columns(&mut self) {
        ...
        // Consider the end of an ongoing animation because that's what compute to fit does too.
        let view_x = self.target_view_main_pos();
            ...
            .flat_map(move |(col, col_pos)| {
                col.tiles_in_render_order()
                    .map(move |(tile, tile_off, visible)| {
                        let pos = axis.point_out(col_pos + tile_off + tile.render_offset());
            ...
            .flat_map(move |(col, col_pos)| {
                col.tiles_in_render_order_mut()
                    .map(move |(tile, tile_off)| {
                        let mut pos = axis.point_out(col_pos + tile_off + tile.render_offset());
                        // Round to physical pixels.
                        if round {
    ...
    pub fn render<R: NiriRenderer>(
        ...
        for (col, col_pos) in self.columns_with_render_positions() {
            ...
            for (tile, tile_off, visible) in col.tiles_in_render_order() {
                let tile_pos =
                    self.map_point_out(col_pos + tile_off + tile.render_offset());
                // Round to physical pixels.
...
impl<W: LayoutElement> Column<W> {
    ...
    fn add_tile_at(&mut self, idx: usize, mut tile: Tile<W>) {
        ...
        self.data.insert(
            idx,
            TileData::new(&tile, WindowHeight::auto_1(), self.axis()),
        );
    ...
    fn update_window(&mut self, window: &W::Id) {
        ...
        tile.update_window();
        self.data[tile_idx].update(tile, axis);
